...
def write_data(data):
    ...
    required_fields = ['user_id', 'device_type', 'ip', 'device_id', 'locale', 'app_version']
    ...
    try:
        cur.execute(
            ...
            (data["user_id"], data["device_type"], data["ip"], data["device_id"], data["locale"], int(data["app_version"]), data["create_date"]),
        )
        conn.commit()
